...
class ConfigView(discord.ui.View):
    ...
    async def show_welcome_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT welcome_channel, welcome_message FROM server_settings WHERE guild_id = ?", (self.guild_id,))
    ...
    async def show_confession_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT confession_channel FROM server_settings WHERE guild_id = ?", (self.guild_id,))
        confession_channel = result[0]["confession_channel"] if result else None
    ...
    async def show_role_requests_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT role_request_channel FROM server_settings WHERE guild_id = ?", (self.guild_id,))
        role_request_channel = result[0]["role_request_channel"] if result else None
    ...
    async def show_role_reactions_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT * FROM role_reactions WHERE guild_id = ?", (self.guild_id,))
    ...
    async def show_xp_system_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT xp_enabled, xp_rate FROM server_settings WHERE guild_id = ?", (self.guild_id,))
        xp_enabled = result[0]["xp_enabled"] if result else False
        xp_rate = result[0]["xp_rate"] if result else 1.0
    ...
    async def show_ticket_system_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT ticket_category, ticket_support_role FROM server_settings WHERE guild_id = ?", (self.guild_id,))
        ticket_category = result[0]["ticket_category"] if result else None
        ticket_support_role = result[0]["ticket_support_role"] if result else None
    ...
    async def show_language_config(self, interaction: discord.Interaction):
        ...
        result = await self.bot.db.query("SELECT language FROM server_settings WHERE guild_id = ?", (self.guild_id,))
        current_language = result[0]["language"] if result else "en"
...
class WelcomeConfigView(discord.ui.View):
    ...
    async def disable_welcome(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET welcome_channel = NULL, welcome_message = NULL WHERE guild_id = ?", (self.guild_id,))
...
class ConfessionConfigView(discord.ui.View):
    ...
    async def disable_confession(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET confession_channel = NULL WHERE guild_id = ?", (self.guild_id,))
...
class RoleRequestsConfigView(discord.ui.View):
    ...
    async def disable_role_requests(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET role_request_channel = NULL WHERE guild_id = ?", (self.guild_id,))
...
class RoleReactionsConfigView(discord.ui.View):
    ...
    async def remove_all_reactions(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("DELETE FROM role_reactions WHERE guild_id = ?", (self.guild_id,))
...
class XPSystemConfigView(discord.ui.View):
    ...
    async def enable_xp_system(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET xp_enabled = 1 WHERE guild_id = ?", (self.guild_id,))
    ...
    async def disable_xp_system(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET xp_enabled = 0 WHERE guild_id = ?", (self.guild_id,))
...
class TicketSystemConfigView(discord.ui.View):
    ...
    async def disable_ticket_system(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET ticket_category = NULL, ticket_support_role = NULL WHERE guild_id = ?", (self.guild_id,))
        await interaction.response.send_message(_("config_system.ticket_system.disabled", interaction.user.id, self.guild_id), ephemeral=True)
...
class LanguageConfigView(discord.ui.View):
    ...
    async def set_english(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET language = 'en' WHERE guild_id = ?", (self.guild_id,))
    ...
    async def set_french(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.bot.db.query("UPDATE server_settings SET language = 'fr' WHERE guild_id = ?", (self.guild_id,))
...
class WelcomeChannelModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.bot.db.query("UPDATE server_settings SET welcome_channel = ? WHERE guild_id = ?", (channel.id, self.guild_id))
...
class WelcomeMessageModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.bot.db.query("UPDATE server_settings SET welcome_message = ? WHERE guild_id = ?", (message, self.guild_id))
...
class ConfessionChannelModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.bot.db.query("UPDATE server_settings SET confession_channel = ? WHERE guild_id = ?", (channel.id, self.guild_id))
...
class RoleRequestChannelModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.bot.db.query("UPDATE server_settings SET role_request_channel = ? WHERE guild_id = ?", (channel.id, self.guild_id))
...
class RoleReactionModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.bot.db.query("INSERT INTO role_reactions (guild_id, emoji, role_id) VALUES (?, ?, ?)", (self.guild_id, emoji, role.id))
...
class XPRateModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await self.bot.db.query("UPDATE server_settings SET xp_rate = ? WHERE guild_id = ?", (rate, self.guild_id))
            await interaction.response.send_message(_("config_system.xp_system.rate_set", interaction.user.id, self.guild_id, rate=rate), ephemeral=True)
        except ValueError:
            await interaction.response.send_message(_("config_system.xp_system.invalid_rate", interaction.user.id, self.guild_id), ephemeral=True)
...
class TicketCategoryModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not category or not isinstance(category, discord.CategoryChannel):
            await interaction.response.send_message(_("config_system.ticket_system.category_not_found", interaction.user.id, self.guild_id), ephemeral=True)
            return
        
        await self.bot.db.query("UPDATE server_settings SET ticket_category = ? WHERE guild_id = ?", (category.id, self.guild_id))
        await interaction.response.send_message(_("config_system.ticket_system.category_set", interaction.user.id, self.guild_id, category=category.name), ephemeral=True)
...
class TicketSupportRoleModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not role:
            await interaction.response.send_message(_("config_system.ticket_system.role_not_found", interaction.user.id, self.guild_id), ephemeral=True)
            return
        
        await self.bot.db.query("UPDATE server_settings SET ticket_support_role = ? WHERE guild_id = ?", (role.id, self.guild_id))
        await interaction.response.send_message(_("config_system.ticket_system.support_role_set", interaction.user.id, self.guild_id, role=role.name), ephemeral=True)
...
class ConfigCog(commands.Cog):
    ...
    @app_commands.describe(
        system="Choose a system to configure"
    )
    async def config(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=_("config_system.main_title", interaction.user.id, interaction.guild_id),
            description=_("config_system.main_description", interaction.user.id, interaction.guild_id),
            color=discord.Color.blue()
        )
