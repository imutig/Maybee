...
class XPSystem(commands.Cog):
    ...
    @app_commands.command(name="synclevelroles", description="Synchroniser les rôles de niveau pour tous les utilisateurs")
    @app_commands.describe(confirm="Tapez 'CONFIRM' pour confirmer la synchronisation")
    async def sync_level_roles(self, interaction: discord.Interaction, confirm: str = ""):
        user_id = interaction.user.id
        guild_id = interaction.guild.id
        ...
        if confirm.upper() != "CONFIRM":
            embed = discord.Embed(
                title="🔄 Synchronisation des rôles de niveau",
                description=(
                    "Cette commande va synchroniser les rôles de niveau pour tous les utilisateurs :\n\n"
                    "• Supprimer tous les rôles de niveau des utilisateurs\n"
                    "• Attribuer uniquement le rôle de niveau le PLUS ÉLEVÉ que chaque utilisateur mérite\n"
                    "• Mettre à jour les rôles pour tous les utilisateurs selon leurs niveaux XP actuels\n\n"
                    "⚠️ **Cette opération peut prendre du temps pour les gros serveurs.**\n\n"
                    "Pour confirmer, utilisez : `/synclevelroles confirm:CONFIRM`"
                ),
        ...
        try:
            ...
            level_roles = await self.bot.db.query(
            ...
            if not level_roles:
                await interaction.followup.send("❌ Aucun rôle de niveau configuré pour ce serveur.")
                return
            ...
            if not users:
                await interaction.followup.send("❌ Aucun utilisateur avec de l'XP trouvé sur ce serveur.")
                return
            ...
            status_embed = discord.Embed(
                ...
                description=f"Traitement de {len(users)} utilisateurs avec {len(level_roles)} rôles de niveau...",
                color=discord.Color.blue()
            )
            ...
            for user_row in users:
                ...
                try:
                    ...
                    for role_row in level_roles:
                        ...
                        if user_level >= role_level and role_level > highest_level:
                    ...
                    if highest_role:
                        # Remove all other level roles from this user
                        roles_to_remove = []
                        for role_row in level_roles:
                            if role_row["role_id"] != highest_role.id:
                                role = interaction.guild.get_role(role_row["role_id"])
                                if role and role in member.roles:
                                    roles_to_remove.append(role)
                        
                        if roles_to_remove:
                            await member.remove_roles(*roles_to_remove, reason="Sync des rôles de niveau - suppression des rôles inférieurs")
                        
                        # Add the highest role if user doesn't have it
                        if highest_role not in member.roles:
                            await member.add_roles(highest_role, reason=f"Sync des rôles de niveau - niveau {user_level}")
                            updated_users += 1
                        elif roles_to_remove:
                            # User already had the highest role but we removed lower ones
                            updated_users += 1
                    
                except Exception as user_error:
                    sync_errors.append(f"Utilisateur {user_id}: {str(user_error)}")
                    continue
            ...
            result_embed.add_field(name="Utilisateurs mis à jour", value=str(updated_users), inline=True)
            result_embed.add_field(name="Total utilisateurs traités", value=str(len(users)), inline=True)
            
            if sync_errors:
                error_text = '\n'.join(sync_errors[:5])
                if len(sync_errors) > 5:
                    error_text += f"\n... et {len(sync_errors) - 5} autres erreurs"
